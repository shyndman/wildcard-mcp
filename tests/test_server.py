...
from wildcard_mcp.server import _CATEGORY_DATA, _CATEGORY_NAMES
...
async def test_randomize_single_item(client: Client):
    """Randomize with count=1 returns a single item from the category."""
    result = await client.call_tool("randomize", {"category": "colors"})
    assert result.data in _CATEGORY_DATA["colors"]["items"]


async def test_randomize_multiple_items(client: Client):
    ...
    assert len(items) == 5
    for item in items:
        assert item in _CATEGORY_DATA["colors"]["items"]
...
async def test_categories_loaded():
    """Verify categories are loaded from test config."""
    assert "colors" in _CATEGORY_NAMES
    assert "animals" in _CATEGORY_NAMES


async def test_category_data_not_empty():
    """Verify category data files have content."""
    for name in _CATEGORY_NAMES:
        items = _CATEGORY_DATA[name]["items"]
